...
From [[citeauthor:&arm-cortex-learn]] documentation ([[citeyear:&arm-cortex-learn]]): "Trace refers to the process of capturing data that illustrates how the components in a design are operating, executing, and performing".
...
The generated trace is then exfiltrated via asynchronous serial using a packet protocol, henceforth referred to as the "trace stream".
These facilities are not disjoint (if they were, no insight into the system could be offered) but because the generation of the trace stream does not influence the wider system per [[citeauthor:&arm-cortex-learn]]'s words,
the trace stream (with system-external collection) is a suitable candidate for exploitation to realize insight into a control system with minimal observer effect.
...
# Enter RTIC.
In order to lighten the implementation burden one may employ [[acrfull:RTIC][Real-Time Interrupt-driven Concurrency (RTIC)]], a [[acrfull:RTOS][real-time operating system (RTOS)]] developed at Luleå Technical University based on the extensively studied [[acrfull:SRP][Stack Resource Policy (SRP)]], which mitigates priority inversions while also offering determinant scheduling of future tasks and message-passing between tasks.
# TODO refer to rauk
RTIC also lends itself to [[gls:static-anal][static analysis]] by help of external tools, further lighening the overhead during implementation.
...
The resulting toolset will be employed to implement a regulator for a non-trivial dynamic system, and the functionality and ergonomics of the toolset evaluated.
