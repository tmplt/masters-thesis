...
#+BEGIN_src latex
  ...
  \begin{tikzpicture}[node distance = 2cm, auto]
    ...
    %% record stack
    \node[block] (tpiu) {\gls{TPIU}, \\ \cref{theory:tpiu}; \\ c.f. \cref{fig:debug-relations}};
    ...
    \path[->] (itm-crate) edge node {\texttt{itm} structures} (record-source);
    ...
    \path[->] (replay-file) edge node {trace metadata; \\ recovery maps; \\ \texttt{itm} structures} (replay-source);
    \draw[->] (itm-crate.south) |- (record-source.east);

    %% trace source anchor
    \node[below=3cm of itm-crate] (mux) {};
    \draw[-] (record-source.south) |- (mux.center);
    \draw[-] (replay-source.south) |- (mux.center);

    %% run_loop
    \node[block, below=4cm of itm-crate] (run-loop) {\texttt{cargo-rtic-scope} \\ run loop};
    \path[->] (mux.center) edge (run-loop);

    %% frontend anchors
    \node[below=1cm of run-loop] (mux-frontend) {};
    \path[-] (run-loop) edge (mux-frontend.center);
    ...
    \draw[->] (mux-frontend.center) -| (frontend1.north);
    \draw[->] (mux-frontend.center) -| (frontend2.north);
    \draw[->] (mux-frontend.center) -| (frontends.north);
    \draw[->] (mux-frontend.center) -| (frontendn.north);
  \end{tikzpicture}
