...
The necessary end-user actions can be summarized by the following commands:
 #+begin_src fundamental
   ...
   $ # Example metadata added to a target RTIC application in a crate named "rtic-scope-example-applications"
   $ cargo metadata --format-version 1 | jq '.packages[] | select(.name == "rtic-scope-example-applications") | .metadata'
   {
     "rtic-scope": {
       "dwt_enter_id": 1,
       "dwt_exit_id": 2,
