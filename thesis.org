...
This section covers the packet types that RTIC Scope exploits and provide graphical representations ([[cref:fig:lts1]]--[[cref:fig:data-trace-packet]]) of these packets, sourced from [[textcite:&arm-rm]].
...
if C is set, the next byte in the stream is a header for the next packet.
...
Both types derive their timestamps from the [[ACRshort:TPIU]] reference clock:
...
[[ACRshort:LTS2]] is a single-byte packet where a relative timestamp that can be represented with 3 bits is provided in the header.
...
[[ACRshort:LTS1]] also contain a 2-bit /TC/ field which describes the relationship of the timestamp value with the corresponding non-timestamp packet.
...
# TODO file bug about ignoring TC. Timestamp should be wrapped in a enum saying what quality the timestamp is.
...
4. data packet 3; and
5. [[ACRshort:LTS2]],
then then [[ACRshort:LTS1]] corresponds to data packet 1;
