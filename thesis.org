...
The ITM packet protocol is a TLV schema. Packets are a multiple of bytes.
First byte is header, which denotes the type and how long...
# TODO just copy the desc. from further down the document

RTIC Scope v0.3.1 concerns itself with six packets:
- /Local timestamp packet format 1/ (LTS1) ::
- /Local timsetamp packet format 2/ (LTS2) ::
- /Global timestamp packet format 1/ (GTS1) ::
- /Global timestamp packet format 2/ (GTS2) :: (48 and 64)
- Exception trace packet ::
- Data trace data value packet format ::

For other packets, refer to [[textcite:&arm-rm Appendix D4]].

#+NAME: fig:lts1
#+CAPTION: LTS1 packet format, copied with permission (?) from [[textcite:&arm-rm Figure D4-4]].
[[file:svgs/lts1.pdf]]

#+NAME: fig:lts2
#+CAPTION: LTS2 packet format, copied with permission (?) from [[textcite:&arm-rm Figure D4-6]].
[[file:svgs/lts2.pdf]]

#+NAME: fig:gts1
#+CAPTION: GTS1 packet format, copied with permission (?) from [[textcite:&arm-rm Figure D4-7]].
[[file:svgs/gts1.pdf]]

#+NAME: fig:gts2-48
#+CAPTION: GTS2 48-bit packet format, copied with permission (?) from [[textcite:&arm-rm Figure D4-8]].
[[file:svgs/gts2-48.pdf]]

#+NAME: fig:gts2-64
#+CAPTION: GTS2 64-bit packet format, copied with permission (?) from [[textcite:&arm-rm Figure D4-9]].
...
#+CAPTION: ARMv7-M Exception/IRQ numbers and names. Redrawn from [[parencite:&arm-rm Table B1-4]].
#+NAME: tbl:irqns
...
:PROPERTIES:
:CUSTOM_ID: theory:threadmode
:END:
[[ACRshort:RTIC]] never executes code in thread-mode; thus, the packet contains no trace information applicable to the [[ACRshort:RTIC]] application...
...
- an exception trace packet (=TracePacket::ExceptionTrace=), the lookup maps generated in [[cref:impl:cargo-rtic-scope:generating-metadata-maps]] are consulted for the task matching the packet's IRQ number.
  ...
  If the trace corresponds to a software task dispatcher or a thread-mode event (see [[cref:theory:threadmode]]) it is ignored because it contains redundant trace information[fn:: TODO elaborate on this].
...
Replaying a trace is similar to that of recording one but instead of sourcing the trace stream from the target it is sourced from a replay file.

# TODO do we touch on --clear-traces?

- =--list= :: which lists all recorded traces;
- =--trace-dir= :: which overrides the default directory in which to look for replay files. This option it the companion piece to =trace --trace-dir=;
