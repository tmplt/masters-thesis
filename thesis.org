...
=handle_packet= does just that: handles a single =TimestampedTracePacket= (henceforth referred to as a /chunk/).
This is done several steps:
1. first, the trace packet is converted to an =api::EventChunk= (see [[cref:impl:api]]) via =TraceMetadata::build_event_chunk=:
   1. ...
