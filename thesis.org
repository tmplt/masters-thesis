...
An [[gls:RTIC]] application is implemented by statically declaring a set of atomic /tasks/ which are executed and scheduled by help of Cortex-M hardware features.
This section summarizes the two different task types --- /hardware tasks/ and /software tasks/ --- that together constitute an [[gls:RTIC]] application.
...
# #+CAPTION: Relationship between ARMv7-M debug peripherals. Copied from [[textcite:&arm-rm Figure C1-1]]. Utilization of the \glsxtrfull{ETM} peripheral is outside of the scope of this thesis; it's functions are disabled in the context of RTIC Scope v0.3.1.
...
The reference clock is either the system clock, an asynchronous [[gls:TPIU]] reference clock, or a /global timestamp clock/.
...
1. <<rtic-trace:conf-protocol>> ensures that the target's [[gls:TPIU]] peripheral supports the requested =protocol= by reading the [[gls:TPIU_TYPE]] [[parencite:&arm-rm C1.10.6]];
2. ensures that the user did not request an invalid [[gls:TPIU]] configuration (i.e. =tpiu_freq: 0= or =tpiu_baud: 0=);
3. <<rtic-trace:conf-exception-trace>> ensures that the target's [[gls:DWT]] peripheral supports exception tracing by reading a zero from the /NOTRCPKT/ bit in the [[gls:DWT_CTRL]] [[parencite:&arm-rm C1.8.7]];
4. <<rtic-trace:nofail-conf>> configures the [[gls:DCB]], [[gls:TPIU]], and [[gls:DWT]] peripherals (partially):
   ...
   4. sets the /EXCTRCENA/ bit in [[gls:DWT_CTRL]] which enables the generation of exception traces in the [[gls:DWT]] [[parencite:&arm-rm C1.8.7]].
5. <<rtic-trace:itm>> Applies [[gls:ITM]]-related options given to =configure= by writing to the [[gls:ITM_TCR]] while also checking for target support for the requested configuration [[parencite:&arm-rm C1.7.6]]:
...
6. <<rtic-trace:dwt>> Configures [[gls:DWT]] comparators for software task tracing:
   ...
   3. configures the first comparator to signal a match (and generate an associated trace stream packet) when data is written to the first watch variable:
      1. writes to seven fields in the [[gls:DWT_FUNCTIONn]][fn:dwt-n] [[parencite:&arm-rm C1.8.17]], where $n$ is the offset of the comparator (in the context of [[cref:lst:rtic-trace:configure]], $n=1$):
         ...
         and 0 to /DATAVSIZE/, /DATAVADDR0/, and /DATAVADDR1/ because these fields are [[gls:SBZ]] [[parencite:&arm-rm Glossary-855]] in address comparison context.
...
Several fields in [[gls:ITM_TCR]] are potentially [[gls:RAZ-WI]], [[gls:RAZ]], or [[gls:RAO]] [[parencite:&arm-rm Glossary-854]] in order to signal hardware support.
...
#+begin_quote
\noindent
A decoder for the [[gls:ITM]] and [[gls:DWT]] packet protocol[fn:: [[gls:DWT]] packets (hardware event packets) are included in the [[gls:ITM]] packet protocol standard.] as specifed in [[[textcite:&arm-rm Appendix D4]]].
...
The ATSAME51N20A is one example of an [[gls:MCU]] that displays this noise, seen in Fig. ???, as a consequence of the configuration done to pin PC27 in [[cref:lst:atsame51n20a-swo-conf]].
\missingfigure{Display the noise seen when configuring SWO on the ATSAM}

#+NAME: lst:atsame51n20a-swo-conf
#+CAPTION: Configuration of the PC27 pin for the ATSAME51N20A [[gls:MCU]] to act as the [[gls:SWO]] pin. This configuration induces transient noise on the pin, seen in Fig. ???.
#+begin_src rust
  #[init]
  fn init(mut ctx: init::Context) -> (SharedResources, LocalResources, init::Monotonics()) {
...
- Storing trace data on-chip :: [[textcite:&arm-rm C1.10]] refers to a [[gls:TPIU]]-alternative dubbed the [[gls:ETB]], described in [[textcite:&coresight]], that stores the trace data in on-chip memory.
...
[[textcite:&arm-rm Fig. C1-1]] can on the other hand be interpreted to imply that both local and global timestamps are derived from this global timestamp clock, but it is nowhere unambiguously stated as such.
