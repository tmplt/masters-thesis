...
The set of /requirements/ RTIC Scope must fulfill within the scope of this thesis follows: RTIC Scope shall be able to
...
3. decode the stream stream to Rust structures;
4. <<req:rtic>> recover RTIC application metadata not contained within the trace stream;
5. associate timestamps to RTIC task events;
6. <<req:report>> report RTIC task events in real-time to the RTIC Scope end-user by use of a defined communication protocol while also offering a reference implementation of a /frontend/ which implements this protocol;
7. record a trace to file which can be replayed offline; and
8. be invoked as a ~cargo~ subcommand.
...
These /delimitations/, which are related to the requirements enumerated in [[#requirements]], are as follows:
1. Requirement [[req:itm-gen]]: RTIC Scope need only apply the device mutations that are common to all ARM Cortex-M targets in order to enable trace stream generation and exfiltration. RTIC Scope must not apply device-specific mutations.
2. Requirement [[req:input]]: RTIC Scope does not have to ensure that a trace stream is read from the device; that responsibility falls upon the RTIC Scope end-user. However, RTIC Scope must fail (or optionally warn the end-user) if it is unable to correctly decode the read data stream.
3. Requirement [[req:rtic]]: RTIC Scope will only need to support RTIC version 1.0.0, see [[textcite:&rtic]]. RTIC Scope must only recover metadata necessary to report the timestamped status of hardware and software tasks, as defined by RTIC.
4. Requirement [[req:report]]: The defined communication protocol must enable one-way communication from the ~cargo~ subcommand /backend/ to the reference /frontend/. The protocol must not enable two-way communication.
