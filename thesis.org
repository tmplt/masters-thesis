...
The [[ACRshort:DWT]] peripheral provides the core of the utilized hardware tracing functionalities by generating packets when
- a configured range of memory is read or written (known as /data tracing/); and
- whenever the processor enters, exits, and returns to  an exception handler (known as /exception tracing/).
In [[textcite:&arm-rm]], the packets the [[ACRshort:DWT]] generate are termed /hardware event packets/.

Because hardware tasks are implemented as exception handlers it is only then natural that exception tracing is utilized to trace the state of these tasks.
Software tasks, however,  are implemented on top of exception handlers.
Because multiple software tasks may be assigned to a single task dispatcher we cannot utilize exception tracing for these tasks.
