...
This is done as a preparatory stage for [[cref:impl:cargo-rtic-scope:generating-metadata-maps]] and [[cref:impl:cargo-rtic-scope:flash-target]];
...
This JSON output describes all artifacts that are built, including the singular binary that the end-user wants to trace, for which ~cargo build~ tells us the path to the built binary we use to flash the target in [[cref:impl:cargo-rtic-scope:flash-target]], and the path to the source file of this binary, which is used in [[cref:impl:cargo-rtic-scope:generating-metadata-maps]].
