...
#+BEGIN_src latex
  ...
  \begin{tikzpicture}[node distance = 1cm, auto]
    ...
    \node[block, below=0.5cm of clock] (itm) {\gls{ITM} \\ (timestamps, muxing), \\ \cref{theory:itm-periph}};
  ...
  %%   \draw[dotted,fill=yellow,fill opacity=0.2] let \p1=(target.north), \p2=($(tpiu.south east)+(0.3,-0.3)$) in (target.north west) rectangle (\x1,\y2);
  ...
  The parallel trace output is briefly covered in this text but not utilized.}
  \end{figure}
#+END_src
