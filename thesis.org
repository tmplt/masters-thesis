...
# Enter RTIC.
In order to lighten the implementation burden one may employ [[gls:RTIC][Real-Time Interrupt-driven Concurrency (RTIC)]], a [[gls:RTOS][real-time operating system (RTOS)]] developed at Luleå Technical University based on the extensively studied [[gls:SRP][Stack Resource Policy (SRP)]], which mitigates priority inversions while also offering determinant scheduling of future tasks and message-passing between tasks [[parencite:&rtic]].
# TODO refer to rauk
...
# TODO: Oh yeah, and Rust
RTIC is written in Rust, which further lightens the load of implementation [[parencite:&rust-lang]].\todo{summarize the embedded aspects}
...
# TODO Summarize what Rust is and why it's used, expand on the embedded aspect touched upon in the introduction.

**** Peripheral Access Crates (PACs)
# TODO What are PACs, what are they used for? svd2rust
