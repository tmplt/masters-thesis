...
* TODO Introduction
Embedded systems --- a unit combination of a central processing unit, system memory, and input/output peripheral devices --- serve a key role in the operation of systems with electronical components (in which the unit is /embedded/) where computations must be made.
Examples of such systems are digital control systems which regulate one or more control quantities such that they are kept within a defined bound, often the goal is to match a reference signal.
A regulator observes the internal states of a system under control via sensors and affect the same system (not always directly) via actuators.
# rocket or jet plane instead?
Examples of regulators are autopilots in aeroplanes where an approach vector to an air field must be maintained;
# nuclear power plant?
and chemical processes where mixtures of multiple substances, not seldom volatile, must be maintained to generate a wanted product.
...
The aeroplane may thus fly on an incorrect heading and put lives in danger, or the chemical plant may explode.
This puts a real-time constrain on the digital controller which greatly limits how it can be implemented.
Further, a digital controller under implementation must be debugged so that the engineers can verify its correct operation, which grows exponentially in difficulty with the numer of tasks that must be done.
Unfortunately, embedded systems are subject to the observer effect: to observe the internal state of a embedded system its operation must be affected.
...
A wish is then to minimize this observer effect while recording the state of the embedded system, and in effect minimizing the effect on the underlying control system.

ARM is a ubiquitous vendor for embedded systems with a number of sub-vendors.
...
Trace generation (with external trace collection) it thus a suitable exploitation candidate for debugging purposes in embedded control systems.
...
The aim of this project is thus to investigate if non-intrusive tracing can be realized and sufficently sufficenty abstracted for "batteries included" usage (minimal overhead for user to utilize) and be readily made available for an emerging RTOS in the form of a tool that fits into the ecosystem.
