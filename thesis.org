...
| DCB  | DCB          | \textit{Device Control Block}                   |
...
:END:
~cortex-m-rtic-trace~ is an auxilliary target-side crate that configures all relevant Cortex-M peripherals --- namely the [[acrfull:DCB]], [[acrfull:TPIU]], [[acrfull:DWT]], [[acrfull:ITM]], -- for tracing.
...
- ~cortex_m_rtic_trace::trace~ :: a macro with with to trace software tasks.
