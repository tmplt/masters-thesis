...
*** Problem definition
This thesis explores the possibility of developing a tool (RTIC Scope) that enables an RTIC application developer to gain non-invasive insight into an RTIC application.
This is done by exploiting the trace generation sub-system (DWT, ITM, and TPIU) of the ARM Cortex-M7 platform and capturing the generated trace stream on a host system for analysis.
The captured trace stream must be decoded, timestamped, and associated to tasks and resources defined in the RTIC application before being presented to the developer.
...
*** Contributions
 ...
 - ~cargo-rtic-scope~ :: a cargo subcommand that acts as daemon:
   it records raw trace data, associates it to timestamped RTIC tasks by help of ~itm-decode~, saves it to file, and forwards it to any spawned frontends.
...
For this reason RTIC Scope shall make it easy to develop front-ends that extend the tool for any end-user's needs.
This shall be done by exposing an JSON API for the end-user.
Additionally, a barebone CLI frontend will be developed as a proof-of-concept and debugging purposes.

Further, the work of this thesis will not stray from the ITM specification. ETM, for example, will not be exploited.
As for RTIC, only v0.6 (in development as of writing) will be considered for the final release of RTIC Scope that occur within the frame of this thesis.
Releasese past those of this thesis will handle future RTIC releases.

The reasons for these delimitations is to focus on the delivery of a robust backend that does the heavy lifting extracing the trace stream from the embedded RTIC application.
...
# DWT -> ITM -> TPIU.
The /Data WatchPoint and Trace/ (DWT) and /Intrumentation Trace Macrocell/ (ITM) are sources of ITM packets which are forwarded to the /Trace Port Interface Unit/ (TPIU) for device exfiltration.

# DWT summary
The DWT unit provides the core of the tracing functionality by generating trace packets when (for example)
- a configured range of data is read or written (data trace), by help of hardware comparators of which there are a limited numbers;
- whenever the processor enters an exception handler and returns from it;
Thus, if the memory address of a variable is known (for example a state variable), an ITM packet can be generated whenever it is updated.
Further, exception packets allows the bookkeeping of the execution states of RTIC hardware tasks.
We will also know when a task preempts another task.
...
- the multiplexing of hardware event packets from the DWT unit with its own instrumentation packets;
...
With the possibility of logging arbitrary data, bookkeeping can be enabled for RTIC software tasks by writing unique identifiers to exclusive stimulus registers in task prologues and epilogues.

The multiplexing functionality then allows the ITM unit to attach a relative timestamp to the generated trace packets. The timestamps are relative to the boot of the device.
# TODO when exactly is time counting started?
From this, a timeline of all the generated events can be constructed.
# TODO add how timestamps relate to packets

For more information on the ITM unit, refer to [[pdf:~/exjobb/docs/DDI0403E_d_armv7m_arm.pdf::709++1.07][DDI0403E_d_armv7m_arm.pdf: Page 709]].

# TPIU summary
The TPIU enables external visibility of the generated trace packets by serializing these over a set of hardware pins.
...
This is done via /itm-decode/ which pattern-matches the packet header and constructs the relevant structure.
...
# TODO include a snippet of how packets are decoded
