...
When the [[gls:RTIC]] application in [[cref:lst:auto-plotted-rtic]] has been traced and then replayed the dummy produces the plot in [[cref:fig:graphical-trace-plot]].

#+NAME: lst:auto-plotted-rtic
#+CAPTION: [[gls:RTIC]] application traced with RTIC Scope and plotted in [[cref:fig:graphical-trace-plot]]. ~app::hardware~ is preempted by the spawned ~app::software~. No resources declared. Included types omitted for brevity.
#+begin_src rust
    ...
    mod app {
        ...
        fn init(mut ctx: init::Context) -> (SharedResources, LocalResources, init::Monotonics()) {
            ...
            // Above pin configuration yields transient noise on the pin.
            // We start RTIC Scope before continuing.
            cortex_m::asm::bkpt();
...
#+NAME: lst:graphical-trace-plot
#+begin_src python :results file :exports results
  # task, start (µs), finish (µs)
  trace = u"""app::hardware,2084,8252
  app::software,3584,4001"""

  import pandas as pd
  import io
  import matplotlib.pyplot as plt

  df = pd.read_csv(io.StringIO(trace), header=None, names=['Task', 'Start', 'End'])
...
#+NAME: fig:graphical-trace-plot
#+CAPTION: Graphical representation of the trace yielded by ~feat/auto-plot~ for [[cref:lst:auto-plotted-rtic]]. ~app::software~ preempts ~app::hardware~ after approximately 3500 ns. At 4000 ns ~app::hardware~ resumes execution. Task context switch overhead not plotted.
#+RESULTS: lst:graphical-trace-plot
[[file:fig.pdf]]
