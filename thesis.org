...
A graphical representation of how data flows during trace recording can be seen in
...
#+BEGIN_src latex
  ...
  \begin{tikzpicture}[node distance = 1cm, auto]
    ...
    \node[block] (tpiu) {TPIU, \\ \cref{theory:tpiu}};
    \node[block, below=of tpiu] (serial-dev) {serial device};
    \node[block, below=of serial-dev] (itm-crate) {\texttt{itm} crate, \\ \cref{impl:itm}};
    \node[block, below=of itm-crate] (recovery) {\texttt{cargo-rtic-scope} \\ recovery procedure, \cref{impl:cargo-rtic-scope:generating-metadata-maps}; \\ trace recording procedure, \cref{impl:cargo-rtic-scope:trace-target}};
    \node[block, left=of itm-crate] (src) {\gls{RTIC} application \\ source file, \cref{impl:cargo-rtic-scope:building-rtic-app}};
    \node[block, right=of itm-crate] (manifest) {\gls{RTIC} application \\ crate manifest metadata, \\ \cref{impl:cargo-rtic-scope:read-manifest}};

    \node[below=1.8cm of recovery] (frontends) {$\cdots$};
    \node[block, left=1cm of frontends] (frontend1) {$1$st frontend};
    \node[block, right=1cm of frontends] (frontendn) {$n$th frontend};

    \path[->]
    (tpiu) edge node {\gls{SWO}} (serial-dev)
    (serial-dev) edge node {byte stream; \\ \cref{theory:itm-proto}} (itm-crate)
    (itm-crate) edge node {\texttt{itm} structures} (recovery)
    (src) edge (recovery.west)
    (manifest) edge (recovery.east);

    \path[->, every node/.style={sloped}]
    (recovery) edge node {\gls{JSON}} (frontend1.north)
    (recovery) edge node {\gls{JSON}} (frontendn.north);
  \end{tikzpicture}
  \caption{Downstream data flow during trace recording in RTIC Scope.\label{fig:rtic-scope-data-flow-record}}
  \end{figure}
#+END_src
