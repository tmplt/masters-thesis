\newglossaryentry{RS-232}{
  name={RS-232},
  description={An electrical (physical level) protocol used to communicate data in chunks of 7 or 8 bits. The protocol is asynchronous in that it does not use a line dedicated for the clocking of the data; the \gls{baud-rate} must be communicated out-of-band. Refer to \textcite[Section 14.7.8]{art}.}
  }
...
\newglossaryentry{task-dispatcher}{
  ...
  description={\gls{software-task} executor abstracted upon an \gls{interrupt} handler and thus effectively a \gls{hardware-task} hidden by the user. The task dispatcher keeps tabs on when \glspl{software-task} should be executed.}
}
