...
\newglossaryentry{daemon}{
  name={daemon},
  description={A program that runs in the background responsible for a task that does not have a defined execution time (i.e., it runs until told to terminate.) An example of a daemon is a web server which serves \glspl{client}. Sometimes also referred to as a \gls{backend}.}
}
...
\newglossaryentry{sigint}{
  name={SIGINT handler},
  description={A \gls{function} that executes when the program receives a SIGINT \gls{signal}.}
}
...
\newglossaryentry{thread}{
  name={thread},
  description={A unit of concurrency available in rich operating systems used to execute multiple \glspl{function} simultaneously.}
  }
...
\newglossaryentry{function}{
  name={function},
  description={A block of statements executed iteratively optionally given a set of parameters. Optionally returns data at the end of execution.}
}
...
\newglossaryentry{overhead}{
  name={overhead},
  description={Necessary work that must be done to enable functionality or start a process (such as emitting \glspl{trace-packet}.)}
}
